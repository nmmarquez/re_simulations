...
rbind(
    mutate(estValuesMix, type="Mix"),
    mutate(estValues, type="Points"),
    mutate(estValuesPoly, type="Poly")) %>%
    filter(obsField) %>%
    select(type, x, y, sdx, obs) %>%
    gather(variable, value, -(type:y)) %>%
...
bind_rows(lapply(list(simPointsOnly, simPPMix, simPolyOnly), function(x){
    data.frame(
        x=mesh.old$loc[,1], y=mesh.old$loc[,2], 
        obs=as.vector(AprojMesh2Mesh %*% x$z$mu),
        sdx=as.vector(AprojMesh2Mesh %*% x$z$sd)) %>%
        cbind(mesh.old$loc %>%
                  SpatialPoints(US.df@proj4string) %>%
                  over(US.df) %>% 
                  select(GEOID))})) %>%
    mutate(model=rep(c("Points", "Mix", "Poly"), each=mesh.old$n)) %>%
    rename(est=obs) %>%
    mutate(true.Val=rep(x_, 3)) %>%
    filter(!is.na(GEOID)) %>%
    group_by(model) %>%
    summarize(
        corr=cor(est, true.Val),
        rmse=mean(sqrt((est-true.Val)^2)),
        cov=mean(((est-1.96*sdx) <= true.Val) & 
                     ((est+1.96*sdx) >= true.Val))) %>%
